...
public class CalificacionesCSVLoader {
  ...
  public void carga(String nombreFichero) throws IOException, CSVCalificacionesException {
  ...
  private void validaCabecera() throws IOException, CSVCalificacionesException {
    ...
    if (! CABECERA.equalsIgnoreCase(cabecera)) {
      throw new CSVCalificacionesException("Cabecera del CSV Errónea: " + cabecera);
    }
  }

  private static void validaEstudiante(String estudiante) throws CSVCalificacionesException {
    // ¿El nombre del estudiante está encerrado entre comillas?
    if (! estudiante.startsWith("\"")) {
      throw new CSVCalificacionesException("Nombre del estudiante no empieza con \": " + estudiante);
    }

    int posFinNombre = estudiante.indexOf("\",", 1);
    if (posFinNombre == -1) {
      throw new CSVCalificacionesException("Nombre del estudiante no termina con \": " + estudiante);
    }
  ...
  private static void validaNotas(String notas) throws CSVCalificacionesException {
    String[] campos = notas.split(","); 
    int numCamposEsperado = CABECERA.split(",").length - 1;
    if (campos.length != numCamposEsperado) {
      throw new CSVCalificacionesException("Número de campos del CSV incorrecto en línea con: " + notas);
    }
    // ¿Son los campos numéricos?
    for (String num: campos) {
      if (! esEntero(num)) {
        throw new CSVCalificacionesException(num + " no es numérico.");
      }
    }
  }
