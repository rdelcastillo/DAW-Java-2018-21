...
public class Calificaciones {
  ...
  public double getNotaMaximaGlobal() {
    double notaMaxima = 0;
    for (Estudiante e: estudiantes) {
   ...
   * @throws XMLCalificacionesException 
   * @throws AddressBookXMLException, IOException 
   */
  public void guardaXML(String nombreFichero) throws IOException, XMLCalificacionesException {
  ...
  public String toString() {
    ...
    for (int i = 0; i < estudiantes.size(); i++) {
      toString += String.format("%2d. ", i+1) // orden del estudiante
                + estudianteToString(estudiantes.get(i)) +"\n";      
    }
    return toString;
  }
  ...
  private String estudianteToString(Estudiante e) {
    ...
    for (int ejercicio = 1; ejercicio <= NUM_NOTAS; ejercicio++) {
      estudiante += String.format("%2d ", e.getNota(ejercicio));
    }
