...
public class Ej15PalindromoSinTildesV3 {
  ...
  public static void main(String args[]) {
    ...
    while (esPalindromo && i<j) {
      if (cadena.charAt(i)==' ') { // Si hay un espacio en la posición i saltamos
        i++;
      }
      else if (cadena.charAt(j)==' ') { // Si hay un espacio en la posición j saltamos
        j--;
      }
      else if (!cadena.substring(i,i+1).equalsIgnoreCase(cadena.substring(j,j+1))) {
        esPalindromo = false;
      }
      else {
        i++;
